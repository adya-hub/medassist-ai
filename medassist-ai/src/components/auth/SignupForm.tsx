...
export default function SignupForm() {
  ...
  const isFormValid = name.trim().length >= 2 && validateEmail(email) && passwordValid && confirm === password;
  ...
  const inputClass = (hasError: boolean) =>
    `w-full px-4 py-3 rounded-xl border text-gray-900 placeholder-gray-400 bg-gray-50/50 outline-none transition-all ${
      hasError
  ...
  return (
    ...
    >
      <div className="mb-8 text-center sm:text-left">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight mb-1">Create an account</h2>
        <p className="text-gray-500 text-sm">Start your health journey with MedAssist AI.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          />
          {nameError && (
            <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle className="w-3.5 h-3.5" /> {nameError}</p>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          />
          {emailError && (
            <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle className="w-3.5 h-3.5" /> {emailError}</p>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          />
          {confirmError && (
            <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle className="w-3.5 h-3.5" /> {confirmError}</p>
          )}
        </div>
